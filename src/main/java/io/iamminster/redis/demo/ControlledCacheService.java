...
public class ControlledCacheService {
    ...
    @Cacheable(cacheNames = "myControlledCache", key = "T(io.iamminster.redis.demo.ControlledCacheService).getCacheKey(#relevant)")
    public String getFromCache(String relevant) {
        return null;
    }

    @CachePut(cacheNames = "myControlledCache", key = "T(io.iamminster.redis.demo.ControlledCacheService).getCacheKey(#relevant)")
    public String populateCache(String relevant, String unrelevantTrackingId) {
        return "this is it again";
    }

    @CacheEvict(cacheNames = "myControlledCache", key = "T(io.iamminster.redis.demo.ControlledCacheService).getCacheKey(#relevant)")
